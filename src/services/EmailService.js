...
const sendEmailCreateOrder = async (
  ...
  orderItems.forEach((item) => {
    listItems += `<div> 
            ...
            ${
              isPaid
                ? `<div>Bạn đã thanh toán bằng hình thức chuyển khoản</div>`
...
const sendEmailConfirmOrder = async (
  ...
  orderItems.forEach((item) => {
    listItems += `<div> 
            ...
            <div>Đơn hàng của bạn đang được vận chuyển</div>
            <b>Chúc quý khách một ngày tốt lành!</b>
        </div>`;
  });
...
module.exports = { sendEmailCreateOrder, sendEmailConfirmOrder };
